...
export function validateInstance(callable: CallableFunction): ValidationFunction {
    return (payload: any): null | string[] => {
...
export function validateOrder(tuple: Tuple<Hybrid<AnySchema>>): ValidationFunction {
    const validators: Validator[] = tuple.map((element) => new Validator(element));
...
export function validateProperties(properties: Record<string, Hybrid<AnySchema>>): ValidationFunction {
    const validations: [string, Validator][] = Object.entries(properties).map(([key, schemas]) => {
...
export function validateSchemas(schemas: Hybrid<AnySchema>): ValidationFunction {
